...
void setup() {
  size(720, 480);
  video = new Capture(this, 640/2, 480/2); //camera window 
...
void draw() {
  ...
  PVector flowregion = opencv.getAverageFlowInRegion(x, y, w, h);
  PVector totalFlow = opencv.getTotalFlow();
  int flowScale = 50;
...
// simple lights - if movement over treshold turn on 
  if (aveFlow.mag() > 1){
    dmxOutput.set(13, 255); //red
    dmxOutput.set(19, 255); //yellow
  }
  // if movement udner treshold turn off 
  else{
    dmxOutput.set(13, 0);
    dmxOutput.set(19, 0);
  }
...
*/
  // STUDIO MODE (5 channels) // changes in intensity as more movement is detected. 
